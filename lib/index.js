...
function normalizeBaseUrl(raw, diagnostic = "baseURL must be an absolute http(s) URL including the /v1 prefix, e.g. http://gw.local:3000/v1") {
...
var NewApiAdapter = class extends LlmAdapter {
   ...
   */
  officialAdapterFor(connection) {
    if (this.officialDelegate !== void 0 && this.officialDelegate.connection === connection) {
  ...
  async discoverModels(request, signal) {
    ...
    if (connection.mode === "official-direct") {
      ...
      const models2 = await Promise.all(listed.map(async (info) => {
        const resolved = await delegate.resolveModel(connection.provider, info.id, signal);
        return {
          id: info.id,
          ...info.name !== info.id ? { name: info.name } : {},
          ...resolved.context?.contextWindow !== void 0 ? { contextWindow: resolved.context.contextWindow } : {}
        };
      }));
      models2.sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
      return models2;
    }
...
var DEFAULT_OFFICIAL_BASE_URL = "https://api.deepseek.com";
...
function expandGroups(config, environment) {
  ...
  return [{
    ...
    ...config.mode === void 0 ? {} : { mode: config.mode },
    ...config.officialBaseURL === void 0 ? {} : { officialBaseURL: config.officialBaseURL },
    ...config.officialApiKeyRef === void 0 ? {} : { officialApiKeyRef: config.officialApiKeyRef },
...
function resolveGroupOptions(group, environment) {
  ...
  const mode = group.mode === "official-direct" ? "official-direct" : "newapi";
  const rawBase = group.baseURL !== void 0 && group.baseURL.trim().length > 0 ? group.baseURL : DEFAULT_BASE_URL;
  if (group.officialApiKeyRef !== void 0 && group.officialApiKeyRef.length > 0 && !isCredentialRefName(group.officialApiKeyRef)) {
    throw new Error(
  ...
  return {
    ...
    mode,
    ...mode === "newapi" ? { baseURL: normalizeBaseUrl(rawBase) } : {
      // Inert placeholder: official-direct requests never touch it.
      baseURL: DEFAULT_BASE_URL,
      officialBaseURL: normalizeBaseUrl(
        group.officialBaseURL !== void 0 && group.officialBaseURL.trim().length > 0 ? group.officialBaseURL : DEFAULT_OFFICIAL_BASE_URL,
        "officialBaseURL must be an absolute http(s) URL, e.g. https://api.deepseek.com"
      ),
      officialApiKeyRef: group.officialApiKeyRef !== void 0 && group.officialApiKeyRef.length > 0 ? credentialRef(group.officialApiKeyRef) : groupOfficialCredRef(group.id)
    },
...
function apply(ctx, config) {
  ...
  const createOfficialAdapter = (connection) => {
    const official = resolveOfficialOptions({
      ...
      ...connection.officialBaseURL !== void 0 ? { baseURL: connection.officialBaseURL } : {},
      ...connection.officialApiKeyRef !== void 0 ? { apiKeyEnv: connection.officialApiKeyRef } : {},
      streamIdleTimeoutMs: connection.streamIdleTimeoutMs
    });
    const resolveOfficialApiKey = async () => {
      ...
      throw new LlmError5(
        `${PKG}: no official API key for provider route "${connection.provider}"; configure it on the NewAPI settings page in dsh web (official-direct mode, credentials reference "${ref}")`,
        "MISSING_CREDENTIAL"
      );
    };
    let userId;
